"""Compute activation energy Ea for PEG and PP using polyBERT model."""
import sys, os
...
from polybert.train_polybert import PolyBERT_SPE_Predictor, EXTRA_COLS
...
def predict_params(smiles, salt_smi, cond):
    ...
    # polyBERT embedding
    emb = torch.from_numpy(polybert.encode([psmiles])).float()
    ...
    extra_raw = torch.tensor(
        ...
        dtype=torch.float)
    extra_norm = ((extra_raw - torch.tensor(extra_mean)) /
                  torch.tensor(extra_std)).unsqueeze(0)
    ...
    ea_kjmol = y_std * EaR_norm * R / 1000
